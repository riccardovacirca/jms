...
import dev.jms.app.sales.dto.ImportSessionDTO;
...
public class ImportSessionDAO
{
  ...
  public ImportSessionDTO findById(String id) throws Exception
  {
    ...
    sql = "SELECT * FROM jms_sales_import_sessions WHERE id = ?";
                     ...
                     int rowCount, String headers, String preview) throws Exception
  {
    ...
    sql =
      "INSERT INTO jms_sales_import_sessions (id, filename, file_path, row_count, headers, preview, status) " +
      "VALUES (?, ?, ?, ?, ?, ?, 'uploaded')";
  ...
  public void updateMapping(String id, String columnMapping) throws Exception
  {
    ...
    sql = "UPDATE jms_sales_import_sessions SET column_mapping = ?, status = 'mapped', updated_at = NOW() WHERE id = ?";
  ...
  public void updateStatus(String id, String status, String errorMessage) throws Exception
  {
    ...
    sql = "UPDATE jms_sales_import_sessions SET status = ?, error_message = ?, updated_at = NOW() WHERE id = ?";
  ...
  public void markCompleted(String id) throws Exception
  {
    ...
    sql = "UPDATE jms_sales_import_sessions SET status = 'completed', completed_at = NOW(), updated_at = NOW() WHERE id = ?";
    db.query(sql, id);
  }
