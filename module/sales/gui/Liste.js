...
class Liste extends LitElement {
  ...
  async _loadList() {
    ...
    this._error   = null;
    try {
      const res  = await fetch(`/api/sales/liste?page=${this._page}&size=${this._size}`);
  ...
  async _saveLista() {
    ...
    const url    = this._editing ? `/api/sales/liste/${this._editing.id}` : '/api/sales/liste';
  ...
  async _doDelete() {
    ...
    this._deleteId = null;
    try {
      const res  = await fetch(`/api/sales/liste/${id}`, { method: 'DELETE' });
      const data = await res.json();
      if (!data.err) {
        this._loadList();
      }
  ...
  async _loadContatti() {
    ...
    try {
      const res  = await fetch(`/api/sales/liste/${this._lista.id}/contatti?page=${this._ctPage}&size=${this._size}`);
      const data = await res.json();
      if (!data.err) {
  ...
  _onAddSearch(e) {
    ...
    this._addTimer = setTimeout(async () => {
      try {
        const res  = await fetch(`/api/sales/contatti/search?q=${encodeURIComponent(this._addSearch)}&page=1&size=8`);
        const data = await res.json();
  ...
  async _addContatto(contattoId) {
    try {
      const res  = await fetch(`/api/sales/liste/${this._lista.id}/contatti`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async _removeContatto(contattoId) {
    try {
      const res  = await fetch(`/api/sales/liste/${this._lista.id}/contatti/${contattoId}`, { method: 'DELETE' });
      const data = await res.json();
      if (!data.err) {
        this._loadContatti();
      }
  ...
  async _setDefault(id) {
    try {
      const res  = await fetch(`/api/sales/liste/${id}/default`, { method: 'PUT' });
      const data = await res.json();
      if (data.err) {
        this._error = data.log;
      } else {
