...
class Contatti extends LitElement {
  ...
  async _loadList() {
    const url = this._search
      ? `/api/sales/contatti/search?q=${encodeURIComponent(this._search)}&page=${this._page}&size=${this._size}`
      : `/api/sales/contatti?page=${this._page}&size=${this._size}`;
    this._loading = true;
    this._error   = null;
    try {
  ...
  async _saveContact() {
    ...
    const url    = this._editing ? `/api/sales/contatti/${this._editing.id}` : '/api/sales/contatti';
  ...
  async _doDelete() {
    ...
    this._deleteId = null;
    try {
      const res  = await fetch(`/api/sales/contatti/${id}`, { method: 'DELETE' });
      const data = await res.json();
      if (!data.err) {
        this._loadList();
      }
