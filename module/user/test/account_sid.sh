#!/bin/bash
#
# Test: account sid - Auto-generated test
...
case "account_sid" in
  auth_generate_password)
    "$CLI_DIR/account_sid.sh"
    ;;
  auth_change_password)
    NEW_PASSWORD="${TEST_NEW_PASSWORD:-newpass}"
    "$CLI_DIR/account_sid.sh" "$USERNAME" "$PASSWORD" "$NEW_PASSWORD"
    ;;
  account_register)
    ...
    "$CLI_DIR/account_sid.sh" "$USERNAME" "$PASSWORD" "$NEW_USER" "$NEW_EMAIL" "$NEW_PASS" "$RUOLO"
    ;;
  *)
    "$CLI_DIR/account_sid.sh" "$USERNAME" "$PASSWORD"
    ;;
esac
