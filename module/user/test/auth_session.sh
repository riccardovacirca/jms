#!/bin/bash
#
# Test: auth session - Auto-generated test
...
case "auth_session" in
  auth_generate_password)
    "$CLI_DIR/auth_session.sh"
    ;;
  auth_change_password)
    NEW_PASSWORD="${TEST_NEW_PASSWORD:-newpass}"
    "$CLI_DIR/auth_session.sh" "$USERNAME" "$PASSWORD" "$NEW_PASSWORD"
    ;;
  account_register)
    ...
    "$CLI_DIR/auth_session.sh" "$USERNAME" "$PASSWORD" "$NEW_USER" "$NEW_EMAIL" "$NEW_PASS" "$RUOLO"
    ;;
  *)
    "$CLI_DIR/auth_session.sh" "$USERNAME" "$PASSWORD"
    ;;
esac
