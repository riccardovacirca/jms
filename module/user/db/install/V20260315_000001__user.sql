...
CREATE TABLE jms_user_roles (
  name  VARCHAR(10) PRIMARY KEY,
  level SMALLINT    NOT NULL
);

INSERT INTO jms_user_roles (name, level) VALUES
  ('user',  1),
  ('admin', 2),
  ('root',  3);

-- Tabella account. password_hash contiene "salt:hash" generato con PBKDF2WithHmacSHA256.
-- ruolo: FK a jms_user_roles.name.
...
CREATE TABLE jms_user_accounts (
  id                   SERIAL       PRIMARY KEY,
  username             VARCHAR(100) UNIQUE NOT NULL,
  password_hash        VARCHAR(255)        NOT NULL,
  ruolo                VARCHAR(10)         NOT NULL DEFAULT 'user' REFERENCES jms_user_roles(name),
...
CREATE TABLE jms_user_refresh_tokens (
  token      VARCHAR(128) PRIMARY KEY,
  account_id INTEGER      NOT NULL REFERENCES jms_user_accounts(id) ON DELETE CASCADE,
...
-- Eliminati in cascata se l'account viene rimosso.
CREATE TABLE jms_user_password_reset_tokens (
  token      VARCHAR(128) PRIMARY KEY,
  account_id INTEGER      NOT NULL REFERENCES jms_user_accounts(id) ON DELETE CASCADE,
  expires_at TIMESTAMP    NOT NULL,
...
-- Eliminati in cascata se l'account viene rimosso.
CREATE TABLE jms_user_auth_pins (
...
(
  ...
  CONSTRAINT fk_jms_users_acct FOREIGN KEY (account_id) REFERENCES jms_user_accounts(id) ON DELETE CASCADE
);
