...
/**
 * Componente di amministrazione delle campagne CRM.
 * Gestisce la vista elenco, il form di creazione/modifica e la gestione delle liste associate.
 * Montato dalla dashboard tramite il tag `crm-admin-campagne`.
 */
class AdminCampagne extends LitElement {
  ...
  async _loadList() {
    ...
    this._error   = null;
    try {
      const res  = await fetch(`/api/campagne?page=${this._page}&size=${this._size}`);
  ...
  async _saveCampagna() {
    ...
    const url    = this._editing ? `/api/campagne/${this._editing.id}` : '/api/campagne';
  ...
  async _doDelete() {
    ...
    this._deleteId = null;
    try {
      const res  = await fetch(`/api/campagne/${id}`, { method: 'DELETE' });
      const data = await res.json();
      if (!data.err) {
        this._loadList();
      }
  ...
  async _loadListe() {
    ...
    try {
      const res  = await fetch(`/api/campagne/${this._campagna.id}/liste?page=${this._ltPage}&size=${this._size}`);
      const data = await res.json();
      if (!data.err) {
  ...
  _onAddSearch(e) {
    ...
    this._addTimer = setTimeout(async () => {
      try {
        const res  = await fetch(`/api/liste?page=1&size=8`);
        const data = await res.json();
        if (!data.err) {
  ...
  async _addLista(listaId) {
    try {
      const res  = await fetch(`/api/campagne/${this._campagna.id}/liste`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async _removeLista(listaId) {
    try {
      const res  = await fetch(`/api/campagne/${this._campagna.id}/liste/${listaId}`, { method: 'DELETE' });
      const data = await res.json();
      if (!data.err) {
        this._loadListe();
      }
...
customElements.define('crm-admin-campagne', AdminCampagne);
