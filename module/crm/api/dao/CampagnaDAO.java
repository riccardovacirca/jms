...
/** DAO per la gestione delle campagne CRM e delle loro liste associate. */
public class CampagnaDAO
{
  ...
  public List<CampagnaDTO> findAll(int page, int size) throws Exception
  {
    ...
    sql =
      ...
      "FROM jms_crm_campagne c " +
      "LEFT JOIN jms_crm_campagna_liste cl ON cl.campagna_id = c.id " +
  ...
  public int count() throws Exception
  {
    ...
    sql = "SELECT COUNT(*) AS n FROM jms_crm_campagne WHERE deleted_at IS NULL";
    rows = db.select(sql);
  ...
  public int insert(CampagnaDTO c) throws Exception
  {
    ...
    sql =
      "INSERT INTO jms_crm_campagne (nome, descrizione, stato) " +
      "VALUES (?, ?, ?) " +
      "RETURNING id";
  ...
  public void update(CampagnaDTO c) throws Exception
  {
    ...
    sql = "UPDATE jms_crm_campagne SET nome = ?, descrizione = ?, stato = ?, updated_at = NOW() WHERE id = ?";
  ...
  public void delete(int id) throws Exception
  {
    ...
    sql = "UPDATE jms_crm_campagne SET deleted_at = NOW() WHERE id = ?";
    db.query(sql, id);
  }
  ...
  public boolean existsByNome(String nome, Integer excludeId) throws Exception
  {
    ...
    if (excludeId != null) {
      sql = "SELECT COUNT(*) AS n FROM jms_crm_campagne WHERE nome = ? AND id != ? AND deleted_at IS NULL";
      rows = db.select(sql, nome, excludeId);
    } else {
      sql = "SELECT COUNT(*) AS n FROM jms_crm_campagne WHERE nome = ? AND deleted_at IS NULL";
  ...
  public List<ListaDTO> findListe(int campagnaId, int page, int size) throws Exception
  {
    ...
    sql =
      ...
      "FROM jms_crm_campagna_liste cl " +
      "JOIN jms_crm_liste l ON l.id = cl.lista_id " +
  ...
  public void addLista(int campagnaId, int listaId) throws Exception
  {
    ...
    sql = "INSERT INTO jms_crm_campagna_liste (campagna_id, lista_id) VALUES (?, ?) ON CONFLICT DO NOTHING";
  ...
  public void removeLista(int campagnaId, int listaId) throws Exception
  {
    ...
    sql = "DELETE FROM jms_crm_campagna_liste WHERE campagna_id = ? AND lista_id = ?";
    db.query(sql, campagnaId, listaId);
  }
