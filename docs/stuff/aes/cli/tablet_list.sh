#!/bin/bash
# GET /api/aes/tablets - Lista configurazioni tablet con filtri opzionali
#
# Usage:
#   ./tablet_list.sh USERNAME PASSWORD [ACCOUNT_ID] [PROVIDER]
#
# Examples:
#   ./tablet_list.sh admin@example.com password123
#   ./tablet_list.sh admin@example.com password123 123
#   ./tablet_list.sh admin@example.com password123 "" savino

set -e

USERNAME="${1:?Errore: USERNAME richiesto}"
PASSWORD="${2:?Errore: PASSWORD richiesta}"
ACCOUNT_ID="${3:-}"
PROVIDER="${4:-}"

API_BASE="${API_BASE:-http://localhost:8080}"
TMP_DIR="/tmp/aes-cli-$$"
mkdir -p "$TMP_DIR"

# Login e salvataggio cookie
echo "[*] Autenticazione in corso..."
LOGIN_RESPONSE=$(curl -s -c "$TMP_DIR/cookies.txt" -X POST "$API_BASE/api/user/auth/login" \
  -H "Content-Type: application/json" \
  -d "{\"username\":\"$USERNAME\",\"password\":\"$PASSWORD\"}")

if ! echo "$LOGIN_RESPONSE" | grep -q '"err":false'; then
  echo "[!] Login fallito:"
  echo "$LOGIN_RESPONSE" | grep -oP '"log":"\K[^"]+' || echo "Errore sconosciuto"
  rm -rf "$TMP_DIR"
  exit 1
fi

echo "[✓] Login riuscito"

# Costruisci query params
QUERY=""
if [ -n "$ACCOUNT_ID" ]; then
  QUERY="?accountId=$ACCOUNT_ID"
fi
if [ -n "$PROVIDER" ]; then
  if [ -n "$QUERY" ]; then
    QUERY="${QUERY}&provider=$PROVIDER"
  else
    QUERY="?provider=$PROVIDER"
  fi
fi

# Lista tablet
echo "[*] Recupero lista tablet..."
RESPONSE=$(curl -s -b "$TMP_DIR/cookies.txt" -X GET "$API_BASE/api/aes/tablets$QUERY" \
  -H "Content-Type: application/json")

# Cleanup
rm -rf "$TMP_DIR"

# Output formattato
if echo "$RESPONSE" | grep -q '"err":false'; then
  echo "[✓] Lista tablet recuperata con successo:"
  echo "$RESPONSE" | python3 -m json.tool 2>/dev/null || echo "$RESPONSE"
else
  echo "[!] Errore nella richiesta:"
  echo "$RESPONSE" | grep -oP '"log":"\K[^"]+' || echo "$RESPONSE"
  exit 1
fi
