...
cmd module install <path>                        # Installa modulo da path relativo a module/ (cartella o .tar.gz)
cmd module dist <path>                           # Pacchettizza modulo (path relativo a module/) → dist/<nome>-<vers>.tar.gz
cmd module uninstall <key>                       # Disinstalla modulo usando il tracker installato (key = nome o ns/nome)
...
./install.sh --vscode               # Install VSCode extensions inside the dev container (code-server)
./install.sh --vscode-host          # Install VSCode extensions on the host machine (code CLI)
...
└── module/              → Empty placeholder — modules added via cmd module install
    └── .gitkeep
```
...
A new installation includes `status` as the only pre-installed frontend module (`gui/src/module/status/`), which calls `/api/status` (returns HTTP 200 no-body on success) and displays "App is running" or "App is not running". It is the `DEFAULT_MODULE`.
...
- `db/install/` — Flyway SQL migrations applied at install time (naming: `V{timestamp}__{modulename}.sql`)
- `db/uninstall/` — SQL scripts executed at uninstall time to DROP tables/objects created by the module
...
- `cli/` — CLI scripts for the module (optional). Two reserved names: `install.sh` (post-install) and `uninstall.sh` (pre-remove). All scripts in `cli/` are invocable via `cmd module cli <module> <script>`
...
```json
{
  ...
  "uninstall_script": "cli/uninstall.sh",
  "install_notice": null
}
```

**Setup scripts** (optional):
- `install_script`: Path relativo allo script bash eseguito dopo l'import del modulo (es. `"cli/install.sh"`). Lo script riceve due argomenti: `$1` = `WORKSPACE` path (es. `/workspace`), `$2` = `MODULE_KEY` (es. `aes` o `cti/vonage`). Usato per setup iniziale come creazione directory, download asset, ecc. Se lo script fallisce (exit code != 0), viene mostrato un warning ma l'installazione continua.
- `uninstall_script`: Path relativo allo script bash eseguito prima della rimozione del modulo (es. `"cli/uninstall.sh"`). Riceve gli stessi argomenti di `install_script`. Usato per cleanup, backup dati, o avvisi all'utente. Se lo script fallisce, viene mostrato un warning ma la rimozione continua.

Gli script in `cli/` sono invocabili anche manualmente: `cmd module cli <module> install` / `cmd module cli <module> uninstall`. `module_export` rileva automaticamente `cli/install.sh` e `cli/uninstall.sh` e li include nel `module.json` generato.
...
```bash
cmd module install auth                   # da cartella top-level
cmd module install auth-1.0.0.tar.gz     # da archivio
cmd module install cti/vonage             # da cartella con namespace
cmd module install cti/vonage --force     # bypassa verifica dipendenze
```
...
4. Copia migration SQL da `db/install/`
5. Inserisce la chiamata `api.routes` in `App.java` dopo `// [MODULE_ROUTES]`, **wrappata in try-catch**: se il modulo non riesce ad avviarsi (es. config mancante) viene loggato un WARN e l'app continua
...
10. Esegue `install_script` (se definito in `module.json`) — atomico con l'installazione; fallimento mostra warning ma non blocca
...
```bash
cmd module uninstall auth
cmd module uninstall cti/vonage
cmd module uninstall auth --force   # bypassa controllo dipendenze inverse
```
...
Esegue `uninstall_script` (se definito in `module.json`) prima di rimuovere i file — atomico con la disinstallazione; fallimento mostra warning ma non blocca.
Rimuove sorgenti Java, GUI, route da `App.java`, entry da `config.js`, Maven profile da `pom.xml`, tracker, e rigenera `installed.json`.
Esegue gli script SQL in `db/uninstall/` per DROP delle tabelle create dal modulo.
Al termine esegue `cmd app clean` (`mvn clean`) per eliminare i file compilati da `target/`, evitando che migration o classi rimosse rimangano attive al prossimo avvio.

**Tracker installati:** dopo ogni install riuscito viene scritto `src/main/resources/module/<key>/module.json` (es. `module/user/module.json` o `module/cti/vonage/module.json`). Le dipendenze vengono verificate cercando per `name` in tutti i tracker. `module uninstall` richiede il tracker per operare.
...
**Moduli migration-only:** moduli senza Java né GUI (es. `audit`). Struttura: solo `db/install/`, `db/uninstall/` e `module.json` con `api: null` e `gui.config: null`. Supportati nativamente da `module_install`.
...
- `user/` — Authentication + account management (login, logout, 2FA, reset/change password, CRUD admin, self-edit); route `/user`; requires: `audit`. Bootstrap accounts created by Flyway migration with default credentials: `root/root123.` (root), `admin/admin123.` (admin), `operator/operator123.` (user). Change passwords after first install.
...
- `cti/vonage/` — Vonage Voice API integration; route `/cti`, namespace `cti`; protected; requires Vonage config + private key; `@vonage/client-sdk` and `@vonage/cli` are installed automatically via `cli/install.sh` post-install script; routes under `/api/cti/vonage/`; requires: `user`. Webhook URLs to configure in Vonage dashboard: Voice Answer URL → `/api/cti/vonage/answer`, Voice Event URL → `/api/cti/vonage/event`, RTC Event URL → `/api/cti/vonage/event`. `CTI_VONAGE_FROM_NUMBER` must be a real Vonage virtual number (11 digits, no `+`) linked to the application. Custom data passed via `client.serverCall(data)` arrives in the answer webhook body as `custom_data` (JSON-serialized string), not as a query parameter. Operator-session TTL: `sessione_ttl TIMESTAMP` in `cti_operatori` is set to `NOW() + 30 minutes` on every `claimOrRenew()` call; the frontend refreshes every 13 minutes keeping it alive. Scheduler job `cti-session-cleanup` (every minute) releases all rows where `sessione_ttl < NOW()`, recovering operators from crashed browsers within 31 minutes max.
