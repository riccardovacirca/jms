...
Prism understands the [OpenAPI](https://developer.vonage.com/en/getting-started/concepts/openapi) specs we publish for each of our APIs, so you can use this approach to work with any of the Vonage APIs.
...
To find the OpenAPI specification for any of our APIs, navigate to that API from the [Documentation homepage](https://developer.vonage.com/en/documentation). Select the API reference from the left hand menu, and use the YAML download button to download the API specification.

![An example of the Download OpenAPI Specification section](https://developer.vonage.com/api/v1/developer/assets/images/api-spec-download.png)
