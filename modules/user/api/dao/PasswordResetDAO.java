...
public class PasswordResetDAO
{
  ...
  public void saveToken(int accountId, String token) throws Exception
  {
    ...
    sql = "INSERT INTO password_reset_tokens (token, account_id, expires_at) VALUES (?, ?, NOW() + INTERVAL '1 hour')";
  ...
  public Integer findValidAccountId(String token) throws Exception
  {
    ...
    sql  = "SELECT account_id FROM password_reset_tokens WHERE token = ? AND expires_at > NOW() AND used = false";
  ...
  public void markUsed(String token) throws Exception
  {
    ...
    sql = "UPDATE password_reset_tokens SET used = true WHERE token = ?";
    db.query(sql, token);
  }
}
