...
class AccountUsersPage extends LitElement {
  ...
  async _save() {
    ...
    const url    = isCreate ? '/api/account' : `/api/account/${this._panel.id}`;
  ...
  _renderForm() {
    const isCreate = this._panel === 'create';
    const title    = isCreate ? 'Nuovo account' : `Modifica ${this._panel.username}`;
    return html`
      <div class="card mb-4">
  ...
  _renderAdminList() {
    return html`
      <div class="d-flex gap-2 mb-3 align-items-center flex-wrap">
        ...
        <button class="btn btn-primary btn-sm" @click=${this._openCreate}>+ Nuovo account</button>
      </div>
...
customElements.define('account-users-page', AccountUsersPage);
