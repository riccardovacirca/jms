...
import {{APP_PACKAGE}}.auth.handler.GeneratePasswordHandler;
...
public class Routes
{
  ...
  {
    paths.add("/api/auth/account/generate-password", new HandlerAdapter(new GeneratePasswordHandler(), ds));
    paths.add("/api/auth/account",                   new HandlerAdapter(new AccountHandler(), ds));
