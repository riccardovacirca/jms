...
/**
 * Form creazione / modifica account.
 *
 * Admin, nessun id  (#/auth/account/edit)      → form vuoto, crea nuovo account.
 * Admin, id presente (#/auth/account/edit/42)  → form precompilato, modifica account 42.
 * Operatore          (#/auth/account/edit)      → form precompilato con i propri dati.
 */
class AuthAccountEditPage extends LitElement {
  ...
  connectedCallback() {
    ...
    // parts: ['auth', 'account', 'edit'] o ['auth', 'account', 'edit', '42']
    const rawId = parts[3] || null;

    if (this._isAdmin) {
      if (rawId) {
        this._targetId = parseInt(rawId, 10);
        this._isNew    = false;
        this._load(this._targetId);
      } else {
        this._targetId = null;
        this._isNew    = true;
  ...
  async _load(id) {
    this._loading = true;
    this._error   = null;
    const r       = await fetch(`/api/auth/account/${id}`);
  ...
  async _save() {
    ...
    const url    = this._isNew ? '/api/auth/account' : `/api/auth/account/${this._targetId}`;
      ...
      <div class="container py-4" style="max-width:560px">
        <div class="d-flex align-items-center gap-3 mb-4">
          <button class="btn btn-outline-secondary btn-sm"
                  @click=${() => { window.location.hash = '/auth/account'; }}>
            ← Indietro
          </button>
              ...
              ${this._isAdmin ? html`
                <div class="mb-3">
                  ...
                  <select class="form-select"
                          .value=${this._form.ruolo || 'operatore'}
...
customElements.define('auth-account-edit-page', AuthAccountEditPage);
